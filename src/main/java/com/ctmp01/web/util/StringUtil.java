...
public class StringUtil {
    ...
    public static String[] getStringArrayByPattern(String str, String pattern) {
        ...
        // java.com.ctmp02.util.regex是一个用正则表达式所订制的模式来对字符串进行匹配工作的类库包
        ...
        while (matcher.find()) {
            for (int i = 0; i < matcher.groupCount(); i++) { // int groupCount()
                // 返回当前查找所获得的匹配组的数量。
                // org.jeecgframework.core.com.ctmp02.util.LogUtil.info(matcher.group(i));
    ...
    public static boolean isJavaClass(Class<?> clazz) {
        ...
        if(clazz.isArray()){
            isBaseClass = false;
                ...
                || clazz.getPackage().getName().equals("java.com.ctmp02.util")) {
            isBaseClass =  true;
        }
        return isBaseClass;
    }
